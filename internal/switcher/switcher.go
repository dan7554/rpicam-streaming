	...
	if audioDevice != "" {
		...
		return fmt.Sprintf(
			"%s %s "+
				...
				"cam. ! rtph264depay ! h264parse ! video/x-h264,stream-format=avc,alignment=au ! avdec_h264 ! videoconvert ! video/x-raw,format=RGBA ! queue ! compositor name=mixer sink_1::xpos=20 sink_1::ypos=20%s ! videoscale ! video/x-raw,width=1920,height=1080 ! videoconvert ! x264enc tune=zerolatency speed-preset=ultrafast key-int-max=30 bframes=0 bitrate=4000 ! video/x-h264,profile=constrained-baseline ! h264parse ! tee name=enc_tee "+
				"overlay_img. ! mixer. %s"+
